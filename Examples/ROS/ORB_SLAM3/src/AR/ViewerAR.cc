    ...
    for(size_t i=0; i<vMPs.size(); i++)
    {
        ...
        if(pMP)
        {
            if(pMP->Observations()>5)
            {
                ...
                cv::Mat wpmat(3, 1, CV_32F);
                wpmat.at<float>(0, 0) = worldPos.x();
                wpmat.at<float>(1, 0) = worldPos.y();
                wpmat.at<float>(2, 0) = worldPos.z();
...
void Plane::Recompute()
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(!pMP->isBad())
        {
            ...
            Xw.at<float>(0, 0) = worldPos.x();
            Xw.at<float>(1, 0) = worldPos.y();
            Xw.at<float>(2, 0) = worldPos.z();
            o+=Xw;
